...
class HealthCheck {
  ...
  async checkCache() {
    ...
    try {
      ...
      try {
        ...
        if (cacheStats) {
          check.details.hitRate = cacheStats.hitRate;
          check.details.memoryUsage = cacheStats.memoryUsage;
          
          // Check hit rate threshold
          if (cacheStats.hitRate < this.thresholds.cache.hitRate) {
            check.status = 'warning';
            check.message = `Cache hit rate (${cacheStats.hitRate}%) below threshold`;
          }
        }
      } catch (statsError) {
        // Cache stats not available, but connection works
        check.details.statsAvailable = false;
      }

      // Check response time threshold
      if (check.responseTime > this.thresholds.cache.responseTime) {
        check.status = check.status === 'warning' ? 'critical' : 'warning';
        check.message = `Cache response time (${check.responseTime}ms) exceeds threshold`;
      }
  ...
  calculateHealthSummary(healthCheck) {
    ...
    healthCheck.summary.healthy = checks.filter(c => c.status === 'healthy').length;
    healthCheck.summary.warning = checks.filter(c => c.status === 'warning').length;
    healthCheck.summary.critical = checks.filter(c => c.status === 'critical').length;
  }
  ...
  async isReady() {
    try {
      ...
      // System is ready if database and cache are healthy
