...
class AlertSystem {
  ...
  initializeDefaultAlertRules() {
    ...
    this.addAlertRule({
      ...
      condition: 'greater_than',
      description: 'API endpoint response time exceeds 2 seconds',
      enabled: true,
    ...
    this.addAlertRule({
      ...
      enabled: false, // Disabled - too aggressive for development
      escalationPolicy: 'critical_escalation'
    });
    ...
    this.addAlertRule({
      ...
      condition: 'greater_than',
      description: 'Database query execution time exceeds 10 seconds',
      enabled: true,
      escalationPolicy: 'critical_escalation'
    });
    ...
    this.addAlertRule({
      ...
      threshold: 95, // 95%
      condition: 'greater_than',
      description: 'Database connection pool utilization exceeds 95%',
      enabled: true,
      escalationPolicy: 'critical_escalation'
    });
    ...
    this.addAlertRule({
      ...
      threshold: 20, // 20%
      condition: 'greater_than',
      description: 'API error rate exceeds 20%',
      enabled: true,
      escalationPolicy: 'critical_escalation'
    });
  ...
  startMonitoring() {
    ...
    if (process.env.NODE_ENV !== 'production' && !process.env.ENABLE_ALERTS) {
      logger.info('Alert system disabled in development for better performance');
      return;
    }

    // Monitor every 2 minutes (reduced frequency to avoid false positives)
    setInterval(() => {
      this.checkAllAlerts();
    }, 120000);
