...
leaseRouter.get("/", async (req, res) => {
  const items = await prisma.lease.findMany({ where: { agencyId: req.user.agencyId } });
  res.json(items);
});
...
leaseRouter.get(":id", async (req, res) => {
  const item = await prisma.lease.findFirst({ where: { id: req.params.id, agencyId: req.user.agencyId } });
...
leaseRouter.put(":id", async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json(parsed.error.flatten());
  const existing = await prisma.lease.findFirst({ where: { id: req.params.id, agencyId: req.user.agencyId } });
  if (!existing) return res.status(404).json({ error: "Not found" });
  const updated = await prisma.lease.update({ where: { id: existing.id }, data: parsed.data });
  res.json(updated);
});
