...
class ResultLimiter {
  ...
  createLimiterMiddleware(options = {}) {
    ...
    return (req, res, next) => {
      ...
      if (enableMemoryMonitoring && memoryBefore.heapUsed > this.criticalMemoryThreshold) {
      ...
      res.json = (data) => {
        try {
          const limitedData = this.limitResults(data, {
            maxResults,
            entityType,
          ...
          if (Array.isArray(data) && Array.isArray(limitedData)) {
            ...
            res.set('X-Result-Limited', (data.length !== limitedData.length).toString());
          }
          ...
          res.set('X-Memory-Usage-MB', Math.round(memoryAfter.heapUsed / 1024 / 1024).toString());
          res.set('X-Memory-Diff-MB', Math.round(memoryDiff / 1024 / 1024).toString());
  ...
  checkRateLimit(agencyId) {
    ...
    // Clean old requests
    const recentRequests = requests.filter(timestamp => timestamp > hourWindowStart);
    this.requestCounts.set(agencyId, recentRequests);

    // Count requests in last minute
    const minuteRequests = recentRequests.filter(timestamp => timestamp > windowStart);
  ...
  forceGarbageCollection() {
    if (global.gc) {
      try {
        ...
        logger.error('Failed to force garbage collection', { error: error.message });
        return false;
      }
    }
    return false;
   ...
   */
  startMemoryMonitoring(intervalMs = 30000) { // 30 seconds
    setInterval(() => {
...
export const createResultLimiter = (options) => resultLimiter.createLimiterMiddleware(options);