...
async function testRedisConnectivity() {
  ...
  try {
    ...
    if (!sessionManager.redis) {
      throw new Error('Session Redis not available');
    }
    
    await sessionManager.redis.setEx(`session:${testKey}`, 5, testValue);
    const sessionResult = await sessionManager.redis.get(`session:${testKey}`);
    await sessionManager.redis.del(`session:${testKey}`);
    ...
    logger.info('Cache testing skipped - cache system disabled');
...
export function loadBalancerHeaders(req, res, next) {
  ...
  // Update response time on finish (just log, don't set headers)
  res.on('finish', () => {
    const responseTime = Date.now() - startTime;
    // Cannot set headers after response is sent, so just log
