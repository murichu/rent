...
import { rateLimitLog, securityLog, SecuritySeverity } from '../utils/logger.js';
...
class EnhancedRateLimiter {
  ...
  async initializeRedisStore() {
    try {
      ...
      logger.warn('Failed to initialize Redis for rate limiting, using in-memory store', {
        error: error.message
      });
    }
  }
  ...
  createRateLimiter(policy, options = {}) {
    const config = {
      ...
      keyGenerator: (req) => {
        ...
        if (userId) {
          return `${options.prefix || 'rl'}:user:${userId}`;
        }
        // For IP-based limiting, use the default key generator to handle IPv6 properly
        return `${options.prefix || 'rl'}:ip:${req.ip}`;
      },
      ...
      handler: (req, res) => {
        ...
        rateLimitLog('rate_limit_exceeded', identifier, req.originalUrl, {
          method: req.method,
        ...
        if (!req.user?.id) { // Only track unauthenticated requests
      ...
      skip: (req) => {
        ...
        if (req.originalUrl?.includes('/health') || req.originalUrl?.includes('/alive')) {
          return true;
        }
  ...
  trackSuspiciousIP(ip, req) {
    ...
    if (this.suspiciousIPs.has(key)) {
      ...
      securityLog('repeated_rate_limit_violation', {
        ip,
        endpoint: req.originalUrl,
        method: req.method,
        userAgent: req.get('User-Agent'),
        timestamp: new Date().toISOString()
      }, SecuritySeverity.HIGH, req.correlationId);
    } else {
  ...
  getAuthLimiter() {
    if (!this.rateLimiters.has('auth')) {
      this.rateLimiters.set('auth', this.createRateLimiter(RateLimitPolicies.AUTH, {
        prefix: 'auth',
  ...
  getPaymentLimiter() {
    if (!this.rateLimiters.has('payments')) {
      this.rateLimiters.set('payments', this.createRateLimiter(RateLimitPolicies.PAYMENTS, {
        prefix: 'payments',
        policyName: 'payments',
  ...
  getCRUDLimiter() {
    if (!this.rateLimiters.has('crud')) {
      this.rateLimiters.set('crud', this.createRateLimiter(RateLimitPolicies.CRUD, {
        prefix: 'crud',
  ...
  getReadLimiter() {
    if (!this.rateLimiters.has('reads')) {
      this.rateLimiters.set('reads', this.createRateLimiter(RateLimitPolicies.READS, {
        prefix: 'reads',
  ...
  getAdminLimiter() {
    if (!this.rateLimiters.has('admin')) {
      this.rateLimiters.set('admin', this.createRateLimiter(RateLimitPolicies.ADMIN, {
        prefix: 'admin',
  ...
  getUploadLimiter() {
    if (!this.rateLimiters.has('uploads')) {
      this.rateLimiters.set('uploads', this.createRateLimiter(RateLimitPolicies.UPLOADS, {
        prefix: 'uploads',
  ...
  getDefaultLimiter() {
    if (!this.rateLimiters.has('default')) {
      this.rateLimiters.set('default', this.createRateLimiter(RateLimitPolicies.DEFAULT, {
        prefix: 'default',
        policyName: 'default',
        allowAdminBypass: true
      }));
  ...
  getSmartLimiter() {
    return (req, res, next) => {
      ...
      // Payment endpoints
      else if (path.includes('/payment') || path.includes('/mpesa') || path.includes('/pesapal')) {
      ...
      // Admin endpoints
      else if (path.includes('/admin') || (req.user?.role === 'admin' && method !== 'get')) {
