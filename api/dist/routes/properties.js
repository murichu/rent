...
propertyRouter.get("/", async (req, res) => {
    ...
    const items = await prisma.property.findMany({ where: { agencyId } });
    res.json(items);
});
propertyRouter.post("/", async (req, res) => {
    ...
    const created = await prisma.property.create({ data: { ...parsed.data, agencyId } });
    res.status(201).json(created);
});
propertyRouter.get(":id", async (req, res) => {
    const agencyId = req.user.agencyId;
    const item = await prisma.property.findFirst({ where: { id: req.params.id, agencyId } });
    if (!item)
...
propertyRouter.put(":id", async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json(parsed.error.flatten());
    const existing = await prisma.property.findFirst({ where: { id: req.params.id, agencyId } });
    if (!existing)
        return res.status(404).json({ error: "Not found" });
