...
const Toast = ({ 
  ...
  const toastStyles = {
    ...
    borderLeft: `${severity === 'critical' ? '6px' : severity === 'high' ? '5px' : '4px'} solid ${config.color}`,
    maxWidth: '400px',
    minWidth: '300px',
    position: 'relative',
    overflow: 'hidden',
...
class ToastService {
  ...
  show(options) {
    const toast = {
      ...
      duration: options.duration !== undefined ? options.duration : 
