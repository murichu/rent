...
class BaseErrorBoundary extends React.Component {
  ...
  storeErrorLocally = (errorReport) => {
    try {
      const existingErrors = JSON.parse(localStorage.getItem('frontend_errors') || '[]');
...
export function useErrorHandler() {
  const reportError = React.useCallback((error, errorInfo = {}) => {
    try {
      ...
      const existingErrors = JSON.parse(localStorage.getItem('frontend_errors') || '[]');
      existingErrors.push(errorReport);
      localStorage.setItem('frontend_errors', JSON.stringify(existingErrors.slice(-10)));

      // Send to backend
      import('../lib/axios').then(({ default: apiClient }) => {
        apiClient.post('/errors/frontend', {
          type: 'frontend_error',
          ...errorReport
        }).catch(err => {
